...
async function createClaim(signer, address, creditScore, dIdentityContract) {
	...
	const tx = await dIdentityContract.connect(signer).setClaim(address, claim);
	await tx.wait();
}
...
async function verifyCreditProof(
	claimer,
	address,
	proof,
	publicSignals,
	...
	const claim = await dIdentityContract.getClaim(claimer, address);
	const dataHash = claim.dataHash;
...
async function main() {
	...
	// Create Claim
	await createClaim(deployer, addr1.address, 700, dIdentityContract);

	// Credit Proof
	const { creditProof, creditPublicSignals } = await createCreditProof(
		addr1,
		700,
		600,
		800
	);
	const creditVerification = await verifyCreditProof(
		deployer.address,
		addr1.address,
		creditProof,
		creditPublicSignals,
